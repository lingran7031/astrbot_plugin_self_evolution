...
插件的所有提示词已从配置文件移至独立的 `prompts.yaml` 文件，方便自定义修改。
...
| 分类 | 说明 |
| :--- | :--- |
| `persona` | 基础人设（锚点、交流准则、反思指令等） |
| `eavesdrop` | 插嘴系统（系统提示、决策逻辑、内心独白） |
| `memory` | 记忆系统（做梦用户/群画像总结） |
| `growth` | 成长系统（婴儿/幼儿/少年/成年阶段提示） |
| `meta` | 元编程（审查Agent系统提示） |
| `profile` | 用户画像（偏好更新、不确定性表达） |
| `boredom` | 无聊机制（傲慢回复列表） |

### 修改方法

1. 编辑 `prompts.yaml` 文件
2. 修改对应的提示词内容
3. 保存文件
4. 重启 AstrBot 或重载插件
...
eavesdrop:
  decision: |
    你现在是 {persona_name}（{persona_title}）...
