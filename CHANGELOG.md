...
- **Phase 1：统一状态模型 + wave 语义**：新增 `engine/reply_state.py`（`ConversationMomentum` 数据类），引入 wave 概念；DAO 新增 5 个字段：`last_bot_message_at`、`last_bot_message_kind`（normal/passive/active/sticker）、`wave_started_at`、`bot_has_spoken_in_current_wave`、`new_user_message_after_bot`；`eavesdropping.py` 所有状态操作改走 `ConversationMomentum` 接口，`sync_framework_reply_state`、`check_engagement`、`process_passive_engagement` 统一通过 `momentum.user_message_arrived()` / `momentum.bot_spoke()` / `momentum.reset_wave()` 更新状态，`momentum.to_dict()` 统一持久化；`from_dict` 兼容旧格式（无新字段时 `wave_started_at` 推断为 `last_message_time`，`bot_has_spoken_in_current_wave` 默认为 False）；关键行为规则：bot 发言后 `bot_has_spoken_in_current_wave = True`；新用户消息到达后 `new_user_message_after_bot = True`；主动插话在 `bot_has_spoken_in_current_wave` 或 `new_user_message_after_bot` 为 True 时拒绝
