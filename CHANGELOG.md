...
- **Phase 1-2：统一状态模型 + wave 语义 + 统一 Recorder**：
  - Phase 1 新增 `engine/reply_state.py`（`ConversationMomentum` 数据类），引入 wave 概念；DAO 新增 5 个字段：`last_bot_message_at`、`last_bot_message_kind`（normal/passive/active/sticker）、`wave_started_at`、`bot_has_spoken_in_current_wave`、`new_user_message_after_bot`；`eavesdropping.py` 所有状态操作改走 `ConversationMomentum` 接口；`from_dict` 兼容旧格式（无新字段时 `wave_started_at` 推断为 `last_message_time`）
  - Phase 2 新增 `engine/reply_recorder.py`（`ReplyRecorder` 类）：所有 bot 发言状态回写统一到 `ReplyRecorder.record()` 和 `ReplyRecorder.record_framework_normal()` 两个入口，不再分散在各处拼 `new_state`；调用方负责 wave 状态机管理（`reset_wave`/`user_message_arrived`/`bot_spoke()`），Recorder 只负责接收已更新的 `momentum` 并持久化；`bot_spoke()` 内部递增 `consecutive_bot_replies`（调用方不手动递增）；`eavesdropping.py` 的 `sync_framework_reply_state`、`check_engagement`、`process_passive_engagement` 全部通过 Recorder 回写
